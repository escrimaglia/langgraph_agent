# This example demonstrates how to use multiple schemas in a state graph. 
# I define a private state schema that includes specific fields, and an output state schema that focuses on the messages exchanged in the conversation. 
# Literal types are used to restrict the possible values for certain fields, ensuring that the state adheres to predefined options.
...
# This schema combines the private state with the messages exchanged in the conversation
...
# This schema defines the output state, which only includes the messages exchanged in the conversation
...
def calling_llm(state: AllStates) -> dict:
    response = llm.invoke(state.messages)
    return {"messages": [response]}

# Node to parse Input Message
def parse_input(state: PrivateState) -> PrivateState:
    """
...
# Build the Graph
builder = StateGraph(AllStates, input_schema=PrivateState, output_schema=OutputState)
...
# Save the Graph
save_graph("multi_schema.png", graph)

# Use the graph with initial messages
try:
    state = graph.invoke({"name": "Ed", "humor": "feliz", "messages": [HumanMessage(content="Hola, puedes decirme el nombre y el humor de la persona?")]})
